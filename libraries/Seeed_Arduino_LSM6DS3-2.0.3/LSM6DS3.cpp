...
float LSM6DS3::calcAccel(int16_t input) {
    float output = (float)input * 0.061 * (settings.accelRange >> 1) / 1000;  //0.000488
    return output;
}
...
float LSM6DS3::calcGyro(int16_t input) {
    uint8_t gyroRangeDivisor = settings.gyroRange / 125.0;
    if (settings.gyroRange == 245.0) {
        gyroRangeDivisor = 2;
    }

    float output = (float)input * 4.375 * ((float)gyroRangeDivisor) / 1000.0;  //0.07 
    return output;
}
